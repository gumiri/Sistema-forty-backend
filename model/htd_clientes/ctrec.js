...
async function insertCtrec(values) {
    try {
        await dbAccess(`INSERT INTO CTREC VALUES (${values})`, db);
        return true;
    }
    catch (e) {
        console.log(e);
...
async function getCtRec(date) {
    try {
        r = await dbAccess(
            ...
            r.DTREMRETCONFENT, r.STWEB, r.CODITPAG, r.PARCIMPOSTOINDIVIDUAL
            from ctrec r
            inner join GCLIENTE c
            on r.CODIC = c.CODIGO
            where r.dtven > '${date}' and r.st = 'A'`, 
            db);
            return r;
    }
    catch (e) {
...
async function getLastCodigouni() {
    try {
        r = await dbAccess(`SELECT MAX(CODIGOUNI) FROM CTREC`, db);
        return r;
    }
    catch (e) {
        console.log(e);
...
module.exports = { getCtRec, insertCtrec, getLastCodigouni };