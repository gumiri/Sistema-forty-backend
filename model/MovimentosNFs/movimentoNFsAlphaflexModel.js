...
const mapMangueiras = require('./mapaPcParaMetrosAlphaflex.json');
const vendasAlphaflex = require('../alphaflex/relatorioMovimentosNfs/vendasMovimentosNf');
const comprasAlphaflex = require('../alphaflex/relatorioMovimentosNfs/compraMateriaPrimaMovimento');
const fs = require('fs');
...
async function loadEstoque(dt1, dt2) {
    ...
    for (let i = 0; i < vendasAlpha.length; i++) {
        ...
        if (venda['ST'] != 'C   ' && venda['CFOPS'] != 2.201 && venda['CODIGNAT'] != 52 && venda['CODIGNAT'] != 28 && venda['CODIGNAT'] != 50 &&/* venda['CODIGNAT'] != 57 &&*/ venda['CODIGNAT'] != 59 && venda['CODIP'] != null /*&& venda['IMP'] != 'S   '*/) {
            ...
            if (venda['GRU1'] != 2 && venda['GRU1'] != 4 && venda['GRU1'] != 5) {
                kg = 0;
                metros = 0;
                vlrPorMetro = 0;
                continue;
